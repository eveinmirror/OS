...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        // LAB4:EXERCISE1 2312321；2313202；2312554
        /*
        ...
        proc->pid = -1;                 // 进程ID设为-1（未分配）
        proc->runs = 0;                 // 运行次数初始化为0
        proc->kstack = 0;               // 内核栈地址初始化为0
        proc->need_resched = 0;         // 不需要重新调度
        proc->parent = NULL;            // 父进程指针设为NULL
        proc->mm = NULL;                // 内存管理结构设为NULL
        memset(&(proc->context), 0, sizeof(struct context)); // 上下文清零
        proc->tf = NULL;                // 陷阱帧指针设为NULL
        proc->pgdir = boot_pgdir_pa;             // 页目录基址设为NULL
        proc->flags = 0;                // 进程标志清零
...
void proc_run(struct proc_struct *proc)
{
    if (proc != current)
    {
        // LAB4:EXERCISE3 2312321；2313202；2312554
        /*
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    ret = -E_NO_MEM;
    // LAB4:EXERCISE2 2312321；2313202；2312554
    /*
     * Some Useful MACROs, Functions and DEFINEs, you can use them in below implementation.
     * MACROs or Functions:
